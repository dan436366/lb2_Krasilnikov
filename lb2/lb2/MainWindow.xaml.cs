...
namespace lb2
{
    ...
    {
        void Execute(TextBox textBox, StackStory history);
    }

    public class ClearCommand : ICommand
    {
        public void Execute(TextBox textBox, StackStory history)
        {
            textBox.Text = "";
        }
    }
    ...
    {
        public readonly StackStory history;

        public ClearLastEntryCommand(StackStory _history)
        {
            history = _history;
        }

        public ClearLastEntryCommand()
        {
        }
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow(StackStory _history)
        {
            history = _history;
        }
        ...
        public MainWindow()
        {
            ...
            history = new StackStory();

            _clearLastEntryCommand = new ClearLastEntryCommand(history);
        ...
        //private string FormatNumberForDisplay(string numberText)
        //{
        //    if (double.TryParse(numberText, out double number))
        //    {
        //        return number.ToString("G10", System.Globalization.CultureInfo.InvariantCulture);
        //    }
        //    else
        //    {
        //        return "Error";
        ...
        {
            if (textInput.Text.Length < 25)
            {
                textInput.FontSize = 28;
            }
            else if (textInput.Text.Length < 36)
            {
                textInput.FontSize = 20;
            }
            else if (textInput.Text.Length < 40)
            {
        ...
        private void Button_Click(object sender, RoutedEventArgs e)
        {
            ...
            switch (strItem)
            {
                ...
                case "◀️":
                    ...
                    break;
                case "CE":
                    _clearLastEntryCommand.Execute(textInput, history);
                    break;
                case "C":
                    _clearCommand.Execute(textInput, history);
                    break;
                case "⌦":
                    _deleteCommand.Execute(textInput, history);
                    break;
                case "=":
                    CalculateExpression();
                    break;
                ...
                case "0":
                    ...
                    else if (!textInput.Text.StartsWith("0") || textInput.Text.Contains('.'))
                    {
                        textInput.Text += strItem;
                    }
                    break;
                case "00":
                    textInput.Text += strItem;
                    if (textInput.Text.StartsWith("00"))
                    {
                        textInput.Text = "0";
                        break;
                    }
